...
public class Materiel_SRV : BICE_SRV<Material_DTO>
{
    ...
    public List<Material_DTO> GetAll()
    {
        ...
        foreach (var materielDAL in materielsDAL)
        {
            ...
            var materielDTO = new Material_DTO()
            {
                ...
                String_Etat_materiel = etat_materiel.Denomination.ToString()
            };
    ...
    {
        var id_categorie = categorieSRV.GetByDenomination(dto.Categorie) == null
            ? categorieSRV.Add(new Categorie_DTO() { Denomination = dto.Categorie }).Id
            : categorieSRV.GetByDenomination(dto.Categorie).Id;

        var id_etat_materiel = etatMaterielSRV.GetByDenomination(EtatMateriel_BLL.EtatMateriel.Stock).Id; //TODO: bll??
    ...
    public List<Material_DTO> UpdateOnInterventionReturnUsedMaterials(List<Material_DTO> dtos)
    {
        ...
        if (dtos != null || dtos.Count == 0)
        {
            foreach (var dto in dtos)
            {
                ...
                {
                    //TODO:Gérer l'erreur...
                    Materiel_BLL materielBll = new Materiel_BLL(materielDAL.Nombre_utilisations, materielDAL.Nombre_utilisations_limite,
